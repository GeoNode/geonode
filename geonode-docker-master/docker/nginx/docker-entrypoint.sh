...
# Certificate Creation: Checks if an autoissued certificate exists or is older than 365 days. If not, it generates a new one using openssl and saves it to /geonode-certificates/autoissued/.
...
#Symbolic Links: Creates a symbolic link for the HTTPS certificate. It checks if a Certbot-generated certificate exists. If it does, it links to the live certificate; otherwise, it links to the autoissued certificate.
...
#envsubst: Substitutes environment variables in the configuration templates (.envsubst files) and writes the result to the actual Nginx configuration files.

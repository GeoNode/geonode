...
for timedeltadays in range(start,end):
    layer_count.clear()
    print(datetime.strptime((datetime.now()-timedelta(days=timedeltadays)).strftime('%d-%m-%Y'),'%d-%m-%Y')) #if minusdays = 1, analyzes downloads day before; because python code is ran early morning

...
def register_service(request):
    ...
    if request.method == "GET":
        ...
        # name = slugify(request.POST.get('name'))
            ...
            if type is None:
                return HttpResponse(
                    'Could not determine server type', status=400)
            ...
            if type in ["WMS", "OWS"]:
                ...
                return _register_harvested_service(
                    url, name, user, password, owner=request.user)
            elif type == "OGP":
...
def _clean_url(base_url):
    ...
    if 'map' in urlparse.parse_qs(urlprop.query):
        map_param = urllib.urlencode(
            {'map': urlparse.parse_qs(urlprop.query)['map'][0]})
...
def _process_wms_service(url, name, type, username,
                         password, wms=None, owner=None, parent=None):
    """
    ...
    try:
        ...
        supported_crs = None
    if supported_crs and re.search(
            'EPSG:900913|EPSG:3857|EPSG:102100|EPSG:102113', supported_crs):
        return _register_indexed_service(
            type, url, name, username, password, wms=wms, owner=owner, parent=parent)
    else:
        return _register_cascaded_service(
            url, type, name, username, password, wms=wms, owner=owner, parent=parent)


def _register_cascaded_service(
        url, type, name, username, password, wms=None, owner=None, parent=None):
    """
...
def _register_indexed_service(
        type, url, name, username, password, verbosity=False, wms=None, owner=None, parent=None):
    """
...
def _register_harvested_service(
        url, name, username, password, csw=None, owner=None):
    """
    ...
    service = Service.objects.create(base_url=url,
                                     ...
                                     abstract=csw.identification.abstract or _(
                                         "Not provided"),
                                     owner=owner)
            ...
            if "WMS" in known_types:
                ...
                try:
                    _register_arcgis_url(
                        ref["url"], None, None, None, parent=csw)
                except Exception as e:
                    logger.error("Error registering %s:%s" %
...
def _register_arcgis_url(url, name, username, password,
...
def _process_arcgis_folder(
        folder, name, services=None, owner=None, parent=None):
    """
    ...
    for service in folder.services:
        ...
        if not isinstance(service, ArcMapService):
            ...
            try:
                if service.spatialReference.wkid in [
                        102100, 102113, 3785, 3857, 900913]:
                ...
                logger.exception(
                    'Error uploading from the service: ' +
                    service.url +
                    ' ' +
                    str(e))
...
def _harvest_ogp_layers(service, maxrecords=10, start=0,
                        totalrecords=float('inf'), owner=None, institution=None):
    """
...
def ajax_service_permissions(request, service_id):
    ...
    if not request.user.has_perm(
            "maps.change_service_permissions", obj=service):
        return HttpResponse(

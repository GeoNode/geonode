...
def map_detail(request, mapid, snapshot=None, template='maps/map_detail.html'):
    ...
    if snapshot is None:
        config = map_obj.viewer_json(request.user, request.session['access_token'])
    else:
        config = snapshot_config(snapshot, map_obj, request.user, request.session['access_token'])
...
def map_json(request, mapid, snapshot=None):
    if request.method == 'GET':
        map_obj = _resolve_map(request, mapid, 'base.view_resourcebase', _PERMISSION_MSG_VIEW)
        return HttpResponse(json.dumps(map_obj.viewer_json(request.user, request.session['access_token'])))
    ...
    if request.method == 'GET' and 'copy' in request.GET:
        ...
        if 'layer' in params:
                ...
                if layer.storeType == "remoteStore":
                    ...
                    maplayer = MapLayer(map=map_obj,
                                        ...
                                        source_params=json.dumps({
                                            "ptype": service.ptype,
                                            "remote": True,
                                            "url": service.base_url+'?access_token='+request.session['access_token'],
                                            "name": service.name}))
                else:
                    maplayer = MapLayer(
                        map=map_obj,
                        name=layer.typename,
                        ows_url=layer.ows_url+'?access_token='+request.session['access_token'],
            ...
            config = map_obj.viewer_json(
                request.user, request.session['access_token'], *(DEFAULT_BASE_LAYERS + layers))

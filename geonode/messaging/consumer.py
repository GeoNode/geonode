...
class Consumer(ConsumerMixin):
    ...
    def on_message(self, body, message):
        logger.debug(f"broadcast: RECEIVED MSG - body: {repr(body)}")
        message.ack()
        self._check_message_limit()

    def on_email_messages(self, body, message):
        logger.debug(f"on_email_messages: RECEIVED MSG - body: {repr(body)}")
    ...
    def on_geoserver_messages(self, body, message):
        logger.debug(f"on_geoserver_messages: RECEIVED MSG - body: {repr(body)}")
    ...
    def on_notifications_messages(self, body, message):
        logger.debug(f"on_notifications_message: RECEIVED MSG - body: {repr(body)}")
        body.get("id")
        body.get("app_label")
        body.get("model")
        body.get("created")
    ...
    def on_geoserver_all(self, body, message):
        logger.debug(f"on_geoserver_all: RECEIVED MSG - body: {repr(body)}")
        message.ack()
    ...
    def on_geoserver_catalog(self, body, message):
        logger.debug(f"on_geoserver_catalog: RECEIVED MSG - body: {repr(body)}")
        try:
            _update_layer_data(body, self.last_message)
            self.last_message = json.loads(body)
        except Exception:
            logger.debug(f"Could not encode message {repr(body)}")
        message.ack()
        logger.debug("on_geoserver_catalog: finished")
        self._check_message_limit()

    def on_geoserver_data(self, body, message):
        logger.debug(f"on_geoserver_data: RECEIVED MSG - body: {repr(body)}")
        try:
            _update_layer_data(body, self.last_message)
            self.last_message = json.loads(body)
        except Exception:
    ...
    def on_layer_viewer(self, body, message):
        logger.debug(f"on_layer_viewer: RECEIVED MSG - body: {repr(body)}")
...
def _wait_for_layer(layer_id, num_attempts=5, wait_seconds=1):
    ...
    for current in range(1, num_attempts + 1):
        ...
        logger.debug(f"Reached maximum attempts and layer {repr(layer_id)} is still not saved. Exiting..."

...
def upload_xml(folder_dir,obj_dl_list):
    for grid_ref_file_name in obj_dl_list.split(" "):
        try:
            ...
            continue
        keyword_text = ""
        if cephobj_resbase.layer.keyword_list():
            keyword_text+="""       <gmd:descriptiveKeywords>\n         <gmd:MD_Keywords>"""
            for eachkeyword in cephobj_resbase.layer.keyword_list():
        ...
        license_text = ""
        if cephobj_resbase.layer.license:
        ...
        temporal_text = ""
        if cephobj_resbase.layer.temporal_extent_start and cephobj_resbase.layer.temporal_extent_end:
            temporal_text += """\n       <gmd:extent>\n         <gmd:EX_Extent>\n           <gmd:temporalElement>\n             <gmd:EX_TemporalExtent>\n               <gmd:extent>\n                 <gml:TimePeriod gml:id="T_01">\n                   <gml:beginPosition>%s</gml:beginPosition>\n                   <gml:endPosition>%s</gml:endPosition>\n                 </gml:TimePeriod>\n               </gmd:extent>\n             </gmd:EX_TemporalExtent>\n           </gmd:temporalElement>\n         </gmd:EX_Extent>\n       </gmd:extent>\n                """%(cephobj_resbase.layer.temporal_extent_start.strftime("%c"),cephobj_resbase.layer.temporal_extent_end.strftime("%c"))

        linkdl_text = ""
        for eachlink in cephobj_resbase.layer.link_set.download():
        ...
        linkows_text = ""
        for eachows in cephobj_resbase.layer.link_set.ows():
     ...
     <gmd:MD_MaintenanceInformation>
       <gmd:maintenanceAndUpdateFrequency>
         <gmd:MD_MaintenanceFrequencyCode codeSpace="ISOTC211/19115" codeList="http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_MaintenanceFrequencyCode" codeListValue="%(MaintenanceFrequency)s">%(MaintenanceFrequency)s</gmd:MD_MaintenanceFrequencyCode>
            ...
            "RCT": cephobj_resbase.layer.restriction_code_type.identifier if cephobj_resbase.layer.restriction_code_type else "",
            "ConstraintsOther": cephobj_resbase.constraints_other,
            "SRTC": cephobj_resbase.layer.spatial_representation_type.identifier if cephobj_resbase.layer.spatial_representation_type else "",
            "LayerCategory": ">\n         <gmd:MD_TopicCategoryCode>"+cephobj_resbase.layer.category.identifier+"</gmd:MD_TopicCategoryCode>" if cephobj_resbase.layer.category else 'gco:nilReason="missing">',
            "BboxX0": cephobj_resbase.layer.bbox_x0,
            "BboxX1": cephobj_resbase.layer.bbox_x1,
            "BboxY0": cephobj_resbase.layer.bbox_y0,
            "BboxY1": cephobj_resbase.layer.bbox_y1,
            "Temporal": temporal_text,
            "SupplementalInformation": cephobj_resbase.layer.supplemental_information,
            ...
            "Statement": "><gco:CharacterString>"+cephobj_resbase.layer.data_quality_statement+"</gco:CharacterString></gmd:statement>" if cephobj_resbase.layer.data_quality_statement else 'gco:nilReason="missing"/>',
        }

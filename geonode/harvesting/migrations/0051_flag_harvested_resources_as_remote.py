...
def flag_harvested_resources_as_remote(apps, schema_editor):
    ...
    # store is a GeoServer grouping: harvested datasets have none (see #14381), but existing
    # rows still carry whatever slugified value pre-dates that fix until their next harvest.
    Dataset.objects.filter(pk__in=harvested.values("geonode_resource_id")).exclude(store__isnull=True).update(
        store=None
    )

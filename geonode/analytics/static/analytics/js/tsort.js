...
(function( $ ){
	...
	$.fn.tableSort = function( options ) {  
		...
		return this.each(function() {
			...
			// Set th hover cursor to pointer
			$(table).find('tr:has(td)').each(function(index) {
				if (index % 2 == 0) {
					$(this).addClass('even');
				}
				else {
			...
			function display_table(data, direction) {
				...
				for ( index = 0; index < data.length; index++ ) {
					var el = data[index];
					if (index % 2 == 0) {
						$(table).find('tr.tsort_id-' + el.id).removeClass('odd').addClass('even');
					}
					else {
						$(table).find('tr.tsort_id-' + el.id).removeClass('even').addClass('odd');
					}
				}
			}

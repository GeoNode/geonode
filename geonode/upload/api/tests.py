...
class UploadApiTests(StaticLiveServerTestCase, APITestCase, URLPatternsTestCase):
    ...
    def login(self, username=GEONODE_USER, password=GEONODE_PASSWD):
        ...
        self.session_id = self.client.cookies['sessionid'].value

        # selenium will set cookie domain based on current page domain
        self.selenium.get(f'{self.live_server_url}/admin/')
        self.selenium.add_cookie(
            {'name': 'sessionid', 'value': self.session_id, 'secure': False, 'path': '/'})
        ...
        self.selenium.find_element_by_xpath(
            '//form[@action="/account/login/"]/button[@type="submit"]').click()
        ...
        logger.error(f" ---- LOGIN RESP: {response.status_code} / {response.content}")

        try:
            self.selenium.refresh()  # need to update page for logged in user
            self.selenium.get(url)
            logger.error(f" --------------- {self.selenium.page_source} --------------- ")
    ...
    #         # lambda driver: 'Profile of' in driver.title
    #     )

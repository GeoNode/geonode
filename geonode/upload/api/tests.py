...
from geonode.upload.models import Upload, UploadParallelismLimit, UploadSizeLimit
...
class UploadParallelismLimitTests(APITestCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_test_1 = User.objects.create(username="user_test_1", email="user_test_1@test.com", password="user_test_1")
        cls.user_test_2 = User.objects.create(username="user_test_2", email="user_test_2@test.com", password="user_test_2")
        cls.group_user_1 = Group.objects.create(name="group_user_1")
        cls.group_user_1_and_2 = Group.objects.create(name="group_user_1_and_2")
        cls.group_user_1.user_set.add(cls.user_test_1)
        cls.group_user_1_and_2.user_set.add(cls.user_test_1)
        cls.group_user_1_and_2.user_set.add(cls.user_test_2)
        default_limit, created = UploadParallelismLimit.objects.get_or_create(
            slug="default_max_parallel_uploads",
        )
        default_limit.max_number = 6
        default_limit.save()
        UploadParallelismLimit.objects.create(
            slug="user_user_test_2",
            max_number=5,
            user=cls.user_test_2,
            group=None,
        )
        UploadParallelismLimit.objects.create(
            slug="user_user_test_1",
            max_number=4,
            user=cls.user_test_1,
            group=None,
        )
        UploadParallelismLimit.objects.create(
            slug="group_group_user_1",
            max_number=3,
            user=None,
            group=cls.group_user_1,
        )
        UploadParallelismLimit.objects.create(
            slug="group_group_user_1_and_2",
            max_number=2,
            user=None,
            group=cls.group_user_1_and_2,
        )
    ...
    def test_list_limits(self):
        ...
        # List as non admin user
        self.client.force_authenticate(user=self.user_test_1)
        non_admin_response = self.client.get(url)
        ...
        self.assertEqual(non_admin_response.wsgi_request.user, self.user_test_1)

        self.assertEqual(anonymous_response.status_code, 200)
        self.assertTrue(anonymous_response.wsgi_request.user.is_anonymous)

        # Response Content
        size_limits = [(size_limit['slug'], size_limit['max_number']) for size_limit in response.json()['upload-parallelism-limits']]
        expected_size_limits = [('default_max_parallel_uploads', 6), ('group_group_user_1', 3), ('group_group_user_1_and_2', 2), ('user_user_test_1', 4), ('user_user_test_2', 5)]
        for size_limit in expected_size_limits:
            self.assertIn(size_limit, size_limits)

        size_limits = [(size_limit['slug'], size_limit['max_number']) for size_limit in non_admin_response.json()['upload-parallelism-limits']]
        expected_size_limits = [('default_max_parallel_uploads', 6), ('group_group_user_1', 3), ('group_group_user_1_and_2', 2), ('user_user_test_1', 4)]
        for size_limit in expected_size_limits:
            self.assertIn(size_limit, size_limits)

        size_limits = [(size_limit['slug'], size_limit['max_number']) for size_limit in anonymous_response.json()['upload-parallelism-limits']]
        expected_size_limits = [('default_max_parallel_uploads', 6)]
        for size_limit in expected_size_limits:
            self.assertIn(size_limit, size_limits)

...
class NormalUserTest(GeoNodeLiveTestSupport):
    ...
    def test_layer_upload(self):
        ...
        if check_ogc_backend(geoserver.BACKEND_PACKAGE):
            ...
            _log(f"3. ------------ {get_sld_for(gs_catalog, saved_layer)} ")
...
class GeoNodeMapTest(GeoNodeLiveTestSupport):
    ...
    def test_layer_upload(self):
        ...
        msg = (f'There were {len(expected_layers)} compatible layers in the directory, '
               f'but only {len(layers)} were sucessfully uploaded')
    ...
    def test_repeated_upload(self):
        ...
        try:
            ...
            msg = f'Expected a different name when uploading {thefile} using overwrite=False but got {uploaded3.name}'

...
class CephStorageClient(object):
    ...
    def bulk_upload_nonthreaded(self, file_path, container=None):
        file_name = os.path.basename(file_path)
        if container is None:
            container = self.active_container_name
        #file_id_name = file_name.split(".")[0]
    ...
    def does_exist(self, file_name, container=None):
        # check if file exists
        # Assume filename format: E123N4567_LAZ_DRM_U1_1.laz

        if self.connection.get_object(container, file_name):
            print 'Already exists'
            print 'Need to delete previous file'
            return True

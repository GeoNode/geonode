...
class GeoNodeUploader:
    ...
    def upload_to_geonode(self, params, files_to_upload, file, errors):
        ...
        if response.status_code in [500, 400, 401, 403]:
            print(f"{file}: Error uploading: {response.content}")
            errors.append(f"{file}: {response.content}")
            return False
        ...
        try:
            ...
            if not exec_id:
                print(f"{file}: Missing execution_id")
                errors.append(f"{file}: Missing execution_id")
                return False
            ...
            if _exec.status.lower() != "finished":
                print(f"{file}: {_exec.log}")
                errors.append(f"{file}: {_exec.log}")
                return False

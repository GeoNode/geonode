...
def delete_from_postgis(name, store):
    """
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT DropGeometryTable ('%s')" % name)
        conn.commit()
        ...
        try:
            ...
            logger.error("Error closing PostGIS conn %s:%s", name, str(e))
        ...
        if self.DATASTORE and settings.DATABASES.get(self.DATASTORE, None):
            ...
            if hasattr(settings, 'SHARD_STRATEGY'):
                if settings.SHARD_STRATEGY:
                    from geonode.contrib.shard_databases.utils import get_shard_database_name

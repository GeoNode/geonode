...
def fixup_style(cat, resource, style):
    ...
    for lyr in layers:
        if lyr.default_style.name in _style_templates:
            ...
            lyr.default_style = cat.get_style(name, workspace=settings.DEFAULT_WORKSPACE)
    ...
    if sld is not None:
        ...
        if style is None:
            try:
                ...
                try:
                    style = cat.get_style(name + '_layer', workspace=settings.DEFAULT_WORKSPACE)
                    overwrite = style or False
                    cat.create_style(name + '_layer', sld, overwrite=overwrite, raw=True, workspace=settings.DEFAULT_WORKSPACE)
                    style = cat.get_style(name + '_layer', workspace=settings.DEFAULT_WORKSPACE)
                except geoserver.catalog.ConflictingDataError as e:
                    msg = ('There was already a style named %s in GeoServer, '

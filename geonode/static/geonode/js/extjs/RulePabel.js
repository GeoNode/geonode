...
gxp.RulePanel = Ext.extend(Ext.TabPanel, {
    ...
    initComponent: function () {
        ...
        /*this.widgetsChart = new gxp.PointSymbolizer({
            symbolizer: this.getTextSymbolizer(),
            attributes: this.attributes,
            fonts: this.fonts,
            listeners: {
        ...
        this.widgetsChart = new gxp.PointExternalSymbolizer({
            symbolizer: this.getPointExternalSymbolizer(),
            attributes: this.attributes,
            listeners: {
                change: function (symbolizer) {
                    this.fireEvent("change", this, this.rule);
        ...
        var chart = {
            ...
            items: [{
                    ...
                    autoHeight: true,
                    checkboxToggle: true,
                    collapsed: !this.hasPointExternalSymbolizer(),
                    items: [
                        this.widgetsChart
                    ],
                    listeners: {
                        collapse: function () {
                            OpenLayers.Util.removeItem(this.rule.symbolizers, this.getPointExternalSymbolizer());
                            this.fireEvent("change", this, this.rule);
                        },
                        expand: function () {
                            this.setPointExternalSymbolizer(this.widgetsChart.symbolizer);
                            this.fireEvent("change", this, this.rule);
                        },
                        scope: this
                    }
        ...
        if (this.getSymbolTypeFromRule(this.rule) || this.symbolType) {
            ...
            if(this.symbolType.toLowerCase() === "polygon"){
     ...
     */
    hasPointExternalSymbolizer: function () {
        console.log("hasPointExternalSymbolizer");
        var pointcandidate, symbolizer;
        for (var i = 0, ii = this.rule.symbolizers.length; i < ii; ++i) {
            pointcandidate = this.rule.symbolizers[i];
            if (pointcandidate instanceof OpenLayers.Symbolizer.Point) {
                console.log("hasPointExternalSymbolizer in");
                symbolizer = pointcandidate;
                break;
            }
        }
        return symbolizer;
     ...
     *  create one.
     */
    getPointExternalSymbolizer: function () {
        console.log("getPointExternalSymbolizer");
        var symbolizer = this.hasPointExternalSymbolizer();
        if (!symbolizer) {
            console.log("getPointExternalSymbolizer not");
     ...
     *  add it.
     */
    setPointExternalSymbolizer: function (symbolizer) {
        var found;
        console.log("setPointExternalSymbolizer");
        for (var i = 0, ii = this.rule.symbolizers.length; i < ii; ++i) {
            pointcandidate = this.rule.symbolizers[i];
            if (this.rule.symbolizers[i] instanceof OpenLayers.Symbolizer.Point) {
                console.log("setPointExternalSymbolizer in");

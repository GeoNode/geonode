...
class RestoreCommandHelpersTests(GeoNodeBaseTestSupport):
    ...
    def test_mandatory_option_failure(self):
        ...
        self.assertIn("Mandatory option", exc.exception.args[0], '"Mandatory option" failure exception expected.')
    ...
    def test_exclusive_option_failure(self):
        ...
        self.assertIn("Exclusive option", exc.exception.args[0], '"Exclusive option" failure exception expected.')
    ...
    def test_mandatory_option_backup_file_failure(self):
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            self.assertIn("not a .zip file", exc.exception.args[0], '"not a .zip file" failure exception expected.')
    ...
    def test_mandatory_option_backup_files_dir_failure(self):
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            self.assertIn("not a directory", exc.exception.args[0], '"not a directory" failure exception expected.')

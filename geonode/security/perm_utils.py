...
def get_users_with_perms(obj, attach_perms=True, with_superusers=True,
    ...
    user_perms = guardian.models.UserObjectPermission.objects.filter(content_type=obj_content_type,object_pk=obj.id)
    for p in user_perms:
...
def get_groups_with_perms(obj, attach_perms=True):
    ...
    obj_content_type = ContentType.objects.get_for_model(obj)
    group_perms = guardian.models.GroupObjectPermission.objects.filter(content_type=obj_content_type,object_pk=obj.id)

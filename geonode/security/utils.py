...
def get_geofence_rules(page=0, entries=1, count=False):
    ...
    try:
        ...
        if count:
            ...
            """
            _url = url + f'rest/geofence/rules.json?page={page}&entries={entries}'
        r = requests.get(_url,
                         headers=_headers,
...
def toggle_layer_cache(layer_name, enable=True, filters=None, formats=None):
    ...
    if settings.OGC_SERVER['default']['GEOFENCE_SECURITY_ENABLED']:
        try:
            ...
            """
            r = requests.get(url + f'gwc/rest/layers/{layer_name}.xml',
                             auth=HTTPBasicAuth(user, passwd))
            ...
            try:
                ...
                r = requests.post(url + f'gwc/rest/layers/{layer_name}.xml',
                                  headers=headers,
                                  data=payload,
                                  auth=HTTPBasicAuth(user, passwd))
                if (r.status_code < 200 or r.status_code > 201):
    ...
    if settings.OGC_SERVER['default']['GEOFENCE_SECURITY_ENABLED']:
        try:
            ...
            curl -v -u admin:geoserver -XDELETE \
                "http://<host>:<port>/geoserver/gwc/rest/layers/geonode:tasmania_roads.xml"
            """
            r = requests.delete(url + f'gwc/rest/layers/{layer_name}.xml',
                                auth=HTTPBasicAuth(user, passwd))
...
def sync_geofence_with_guardian(layer, perms, user=None, group=None, group_perms=None):
    ...
    if _disable_layer_cache:
        # delete_layer_cache(f'{_layer_workspace}:{_layer_name}')
        filters = None
        formats = None
        # Re-order dictionary

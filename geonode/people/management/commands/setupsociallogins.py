    ...
    def add_arguments(self, parser):
        for provider_name, provider_id in self.get_social_providers():
            ...
            parser.add_argument(
                f"--{client_id_arg}",
                help=(
                    f"Specify the client id for {provider_name}. You may also specify the {self._get_client_id_env(provider_name)} environment variable instead"
                )
            )
            ...
            parser.add_argument(
                f"--{client_secret_arg}",
                help=(
                    f"Specify the secret key for {provider_name}. You may also specify the {self._get_client_secret_env(provider_name)} environment variable instead"
                )
            )
        ...
        if len(social_providers) > 0:
            for provider_name, provider_id in social_providers:
                ...
                if all((client_id, client_secret)):
                    ...
                    self.stdout.write(
                        f"Provider {provider_name} not all params were specified, skipping..."
                    )
        else:
            self.stdout.write(

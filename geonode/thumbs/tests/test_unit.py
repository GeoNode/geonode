...
from geonode.services.signals import post_save_service
...
class ThumbnailsUnitTest(GeoNodeBaseTestSupport):
    ...
    fixtures = GeoNodeBaseTestSupport.fixtures.copy() + [
        'geonode/thumbs/tests/fixtures/resource_base.json',
        'geonode/thumbs/tests/fixtures/upload_session.json',
        'geonode/thumbs/tests/fixtures/service.json',
        'geonode/thumbs/tests/fixtures/style.json',
        'geonode/thumbs/tests/fixtures/layer.json',
        'geonode/thumbs/tests/fixtures/map.json',
        'geonode/thumbs/tests/fixtures/map_layer.json'
    ]
    ...
    def test_generate_thumbnail_name_layer(self):
        ...
        self.assertIsNotNone(re.match(f'layer-{self.re_uuid}-thumb.png', layer_name, re.I), "Layer name should meet a provided pattern")
    ...
    def test_generate_thumbnail_name_map(self, uuid_mock, layers_mock):
        ...
        map_name = thumbnails._generate_thumbnail_name(Map.objects.first())
        self.assertIsNotNone(re.match(f'map-{self.re_uuid}-thumb.png', map_name, re.I), "Map name should meet a provided pattern")
    ...
    def test_layers_locations_composition_map_default_bbox(self):
        ...
        expected_locations = [['http://localhost:8080/geoserver/', ['geonode:theaters_nyc', 'geonode:Meteorite_Landings_from_NASA_Open_Data_Portal1']], ['http://www502.regione.toscana.it/wmsraster/com.rt.wms.RTmap/wms?map=wmsgeologia&map_resolution=91&', ['rt_geologia.dbg_risorse_minerarie']]]

...
@register.simple_tag
def filter_annotatios(map_datasets):
    for dataset in map_datasets:
        dataset_params = json.loads(dataset.dataset_params)
        if dataset_params['type'] == 'vector':
            map_datasets = map_datasets.exclude(id=dataset.id)
    return map_datasets

...
def create_single_dataset(name, keywords=None, owner=None, group=None, **kwargs):
    ...
    ll = (name, "lorem ipsum", name, f"geonode:{name}", [0, 22, 0, 22], test_datetime, ("populartag",), "farming")
...
def create_single_map(name, owner=None, **kwargs):
    ...
    if created:
        ...
        admin.save()
    test_datetime = datetime.strptime("2020-01-01", "%Y-%m-%d")
    ll = (name, "lorem ipsum", name, f"{name}", [0, 22, 0, 22], test_datetime, ("populartag",))

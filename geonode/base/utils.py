...
def remove_duplicate_links(resource):
    ...
    if isinstance(resource, Layer):
        # fixup Legend links
        layer = resource
        legend_url_template = (f"{ogc_server_settings.PUBLIC_LOCATION}ows?service=WMS&request=GetLegendGraphic&format=image/png"
                               f"&WIDTH=20&HEIGHT=20&LAYER={{alternate}}&STYLE={{style_name}}&legend_options=fontAntiAliasing:true;fontSize:12;forceLabels:on")

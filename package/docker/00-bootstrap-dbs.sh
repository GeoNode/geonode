...
# geoserver. So $DB_ADMIN_USER and $DB_ADMIN_PASSWORD should correspond to user that can create database.
...
if psql --no-password -lqt postgres | cut -d \| -f 1 | grep -qw $DB_GEONODE_NAME; then
    ...
    # load sample data and admin account conditionally
    if [ "$IGNORE_DATA" != "TRUE" ]; then
        echo "Loading sample data and admin:admin account"
